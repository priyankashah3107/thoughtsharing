...
const Post = ({ post }) => {
	...
	const { mutate: deletePost, isPending: isDeleting } = useMutation({
		...
		onSuccess: () => {
			...
			// Invalidate the Query to refetch the data, bcz it will refetch all posts
			// instead update the cache from the post
		...
		onSuccess: (updatedLikes) => {
			queryClient.setQueriesData(["posts"], (oldData) => {
				if (!Array.isArray(oldData)) {
					console.error("Expected posts data to be an array", oldData);
					return oldData;
				}
	
				return oldData.map((postLikes) => {
					if (postLikes._id === post._id) {
						return { ...postLikes, likes: updatedLikes };
					}
					return postLikes;
				});
			});
